...
export default function GridItem ({pokemon, gen, addToMyList, removeFromList, listCheck}: GridItemProps) {
  ...
  return (
    <>
    <div key={pokemon.id} className='flex flex-col justify-center items-center bg-box rounded-lg p-2 shadow-xl text-background hover:cursor-pointer'>
      <h1 className='font-bold text-xl'>{capitalize(pokemon.name)}</h1>
      {showInfo ?  
      (<div className='flex flex-col items-center justify-center text-center h-[150px] font-bold text-sm sm:text-base'>
      ...
      <div className='hover:cursor-pointer flex justify-between w-full'> 
        <HiInformationCircle size={25} onClick={() => setShowInfo(!showInfo)}/>
        {gen === 'list' ? (
          <HiOutlineXCircle size={25} onClick={() => removeFromList(pokemon)} />
          ) : listCheck(pokemon) ? (
          <HiHeart size={25} onClick={() => removeFromList(pokemon)} />
          ) : (
          <HiOutlineHeart size={25} onClick={() => addToMyList(pokemon)}/>
        )}
      </div>
    </div>
    </>
